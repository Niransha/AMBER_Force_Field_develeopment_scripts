...
DEST_DIR="/mnt/rna/home/nkumarachchi2019/SCRIPTS/all_bash_sss"

mkdir -p "$DEST_DIR"

find "$SRC_DIR" -type f -name "*.sh" -user nkumarachchi2019 | while read file; do
    cp "$file" "$DEST_DIR/$(basename "$file")_$(date +%Y%m%d_%H%M%S)"
done

echo "All bash scripts copied."
