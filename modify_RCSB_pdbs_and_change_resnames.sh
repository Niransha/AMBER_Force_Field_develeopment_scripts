...
cat tmp2.pdb | awk '{ if ($1=="TER") {printf "TER\n"} else  printf ("%-6s%5d %4s %3s  %4d    %8.3f%8.3f%8.3f \n", $1, $2, $3, $4, $5, $6, $7, $8) }' > tmp3.pdb # this is now looks loke nab pdb. so then follwoing script

sed  -e 's/DG/ G/g' -e  's/DC/ C/g' -e 's/DT/ U/g' -e 's/DA/ A/g' tmp3.pdb > resname.pdb  # /_G/ space is important to keep the format^
 

#perl ~/scripts/create_resname_modify_duplex_rna.pl resname.pdb   # give correctedresname.pdb # see all_in_one_RCSB_psb.sh sctipt, # will do in all_in_one_RCSB_pbs.sh script 
#perl perl ~/scripts/create_newRESNAMEs.pl resname.pdb  # for single stranded pdbs not double helix
